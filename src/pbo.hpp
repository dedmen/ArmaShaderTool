...
class PboFTW_FromReader : public PboFileToWrite {
    std::shared_ptr<PboReader> reader;
public:
    PboFTW_FromReader(std::string filename, std::shared_ptr<PboReader> reader_) : reader(std::move(reader_)) {
        entryInfo.name = std::move(filename);
        //only backslash in pbo
        auto& readerFiles = reader->getFiles();

        auto found = std::find_if(readerFiles.begin(), readerFiles.end(), [filename = entryInfo.name](const PboEntry& file) {
            return file.name == filename;
        });

        if (found == readerFiles.end()) __debugbreak();

        entryInfo.data_size = entryInfo.original_size = found->original_size;
    }
    void writeDataTo(std::ostream& output) override;
};
...
public:
    ...
    const std::vector<std::shared_ptr<PboFileToWrite>>& getFiles() const { return filesToWrite; }
