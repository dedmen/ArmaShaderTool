...
void PboFTW_FromReader::writeDataTo(std::ostream& output) {

    auto& readerFiles = reader->getFiles();

    auto found = std::find_if(readerFiles.begin(), readerFiles.end(), [filename = getEntryInformation().name](const PboEntry& file) {
        return file.name == filename;
    });

    if (found == readerFiles.end()) __debugbreak();

    auto fileBuf = reader->getFileBuffer(*found);

    std::istream inp(&fileBuf, std::ifstream::binary);
